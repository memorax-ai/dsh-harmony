...
需要 Node.js `^22.15.0` 或 `>=23.5.0`，以及当前版本的 `@deepseek-ai/dsh`。Harmony 0.8.13 同时支持旧版导入即执行的入口和新版 `runCli()` 入口，并在发现 profile 时规范化 Windows 命名空间路径。旧版集成区间仍限定在 `0.1.2` 之前，新版区间放宽为 `>=0.1.2-alpha.4 <0.1.7-0`，包含预发布版本。区间内可直接兼容的版本不会仅因版本号变化而要求升级 Harmony。区间外仍会尝试应用，并通过版本漂移警告和精确匹配检查报告结果，不阻止安装；上游接口或补丁目标发生变化时，再升级 Harmony 获取对应适配。
