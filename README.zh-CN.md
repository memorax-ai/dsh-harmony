...
需要 Node.js `^22.15.0` 或 `>=23.5.0`，以及当前版本的 `@deepseek-ai/dsh`。内置 DSH 集成已验证至 `0.1.1-rc.2`。Harmony 不会在安装阶段限制 DSH 版本；Patch 目标版本范围仅用于提示，对更新的版本仍会实际尝试应用，并通过可见的版本漂移警告与精确匹配检查报告结果。
