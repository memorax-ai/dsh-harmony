...
需要 Node.js `^22.15.0` 或 `>=23.5.0`，以及当前版本的 `@deepseek-ai/dsh`。Harmony 0.8.11 同时支持旧版导入即执行的入口和新版 `runCli()` 入口，并在发现 profile 时规范化 Windows 命名空间路径。旧版集成区间仍限定在 `0.1.2` 之前，新版区间放宽为 `>=0.1.2-alpha.4 <0.1.7-0`，包含预发布版本。区间内可直接兼容的版本不会仅因版本号变化而要求升级 Harmony。区间外仍会尝试应用，并通过版本漂移警告和精确匹配检查报告结果，不阻止安装；上游接口或补丁目标发生变化时，再升级 Harmony 获取对应适配。

回归基线为 `0.1.1-rc.2`。隔离上游检查覆盖 `0.1.2-rc.1`、`0.1.5-rc.2` 和 `0.1.6-alpha.2`，验证 Web 启动、5 个内置补丁无版本警告地绑定、在线 CLI 状态，以及客户端补丁禁用/启用时的热重载。构建后，将 `DSH_HARMONY_DSH_ENTRY` 指向隔离安装的 `@deepseek-ai/dsh/lib/bin.js`，即可运行 `npm run test:upstream`。该检查不覆盖浏览器交互或模型对话。
