...
Requires Node.js `^22.15.0` or `>=23.5.0` and a current `@deepseek-ai/dsh` installation. Harmony 0.8.12 supports both legacy self-executing DSH entries and newer `runCli()` entries, and normalizes Windows namespaced paths during profile discovery. The legacy integration lane stays below `0.1.2`; the modern lane covers `>=0.1.2-alpha.4 <0.1.7-0`, including prereleases. Compatible releases in these lanes need no Harmony upgrade merely because their version changes. Versions outside the lanes are still attempted with visible drift warnings and exact-match checks; installation is not blocked. If upstream changes an interface or Patch target, upgrade Harmony for the corresponding adapter.

The regression baseline is `0.1.1-rc.2`. Isolated upstream checks cover `0.1.2-rc.1`, `0.1.5-rc.2`, and `0.1.6-alpha.2`: Web startup, all five built-in Patches binding without version warnings, live CLI status, and client Patch disable/enable with hot reload. Run `npm run test:upstream` after building, with `DSH_HARMONY_DSH_ENTRY` pointing to an isolated installation's `@deepseek-ai/dsh/lib/bin.js`. This check does not exercise browser interactions or model conversations.
