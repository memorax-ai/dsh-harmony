...
Requires Node.js `^22.15.0` or `>=23.5.0` and a current `@deepseek-ai/dsh` installation. Harmony 0.8.11 supports both legacy self-executing DSH entries and newer `runCli()` entries, and normalizes Windows namespaced paths during profile discovery. The legacy integration lane stays below `0.1.2`; the modern lane covers `>=0.1.2-alpha.4 <0.1.7-0`, including prereleases. Compatible releases in these lanes need no Harmony upgrade merely because their version changes. Versions outside the lanes are still attempted with visible drift warnings and exact-match checks; installation is not blocked. If upstream changes an interface or Patch target, upgrade Harmony for the corresponding adapter.
